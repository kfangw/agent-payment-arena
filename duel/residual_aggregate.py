...
Run:  python -m duel.residual_aggregate --results results_r4
"""
...
def _load(cell, results):
    ...
    return dict(cell=cell, seed=seed, b=b, counts=counts,
                mexp=p["mean_exposure"], repro=p["a2_base_repro_gap"],
                r4b=p["r4b"], hash=d["params_hash"], code=d["code"],
...
def _loss(r, minus, sub):
    ...
    lo, hi = boot_ci(diff, r["counts"], n_boot=N_BOOT, seed=r["seed"],
                     level=SIM_LEVEL)
...
def build(results="results_r4"):
    ...
    for cell in CELL_SEED:
        ...
        r["loo_sum"] = sum(r["losses"][x]["mean"] for x in
                           ("A-r", "A-l", "A-i", "A-v"))
        rows.append(r)
    return rows
...
def main(argv=None):
    ...
    print("== R4a: ablation losses (A - A_x), $ per payment and bp, "
          f"3-cell simultaneous {SIM_LEVEL:.4f} ==")
    hdr = ["cell", "resid", "bp"] + ABL
    print("%-14s %8s %6s" % (hdr[0], "A-B3", "bp")
          + "".join("%9s" % x for x in ABL))
    for r in rows:
        print("%-14s %8.4f %6.1f" % (r["cell"], r["resid"]["mean"],
                                     r["resid"]["bp"])
              + "".join("%9.4f" % r["losses"][x]["mean"] for x in ABL))
    print("\n-- decomposition (identity): (A - A-ilrv) + (A-ilrv - B3) = A - B3 --")
    for r in rows:
        print(f"{r['cell']:14} extra(A-A_ilrv)={r['extra']['mean']:.4f} "
    ...
    for r in rows:
        print(f"{r['cell']:14} loo_sum(r+l+i+v)={r['loo_sum']:.4f} "
              f"vs A-B3={r['resid']['mean']:.4f}")
    ...
    for r in rows:
        ...
        for c in top_cells(m):
            print(f"    r={c['r']} v{c['v_bin']} pi{c['pi_bin']} | "
