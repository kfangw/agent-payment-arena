...
CELL_SEED = {
    "E-fast x F1": 1, "E-fast x F2": 2, "E-fast x F3": 3,
    "E-slow x F1": 4, "E-slow x F2": 5, "E-slow x F3": 6,
    "E-outage x F1": 7, "E-outage x F2": 8, "E-outage x F3": 9,
}
...
def load(cell, results, results_b4):
    ...
    return dict(
        ...
        diff=pp["a2_minus_b4"]["mean"], ci=pp["a2_minus_b4"]["ci95"],
        perm_p=pp["a2_minus_b4"]["perm_p"],
        mean_exposure=pp["mean_exposure"],
        base_code=base["code"], b4_code=b4["code"],
        base_hash=base["params_hash"], b4_hash=b4["params_hash"],
...
def main(argv=None):
    ...
    hdr = ("cell", "A", "B1", "B3", "B4", "k*", "A-B4", "ci95", "bp",
           "holm_p", "verdict")
    print("%-14s %7s %7s %7s %7s %4s %8s %18s %7s %8s %s" % hdr)
    for r in rows:
        print("%-14s %7.3f %7.3f %7.3f %7.3f %4d %8.4f [%7.4f,%7.4f] %7.2f %8.4f %s"
              % (r["cell"], r["A2"], r["B1"], r["B3"], r["B4"], r["k"],
                 r["diff"], r["ci"][0], r["ci"][1], r["bp"], r["holm_p"],
                 r["verdict"]))
