...
from .outage import (compile_outage, draw_outage_batch_crn, replay_outage,
                     retime_answers_geom, survival, window_AD)
...
def _chain_ctx(env, rho, flow, seed, n_tune, n_eval):
    ...
    return dict(env=env, rho=rho, flow=flow, ex=ex, q_hat=q_hat,
                rho_hat=rho_hat, a2=a2, b1=b1, grid=grid,
                tune_d=tune_d, u_tune=u_tune, eval_d=eval_d, u_eval=u_eval,
...
def _outage_ctx(env, flow, seed, n_tune, n_eval, ppe=50):
    rng = np.random.default_rng(seed)
    tune_d, u_tune = draw_outage_batch_crn(env, flow, n_tune, rng,
                                           payments_per_episode=ppe)
    eval_d, u_eval = draw_outage_batch_crn(env, flow, n_eval, rng,
                                           payments_per_episode=ppe)
    ...
    return dict(env=env, rho=env.rho, flow=flow, ex=ex, q_hat=q_hat,
                rho_hat=rho_hat, a2=a2, b1=b1, grid=grid,
                tune_d=tune_d, u_tune=u_tune, eval_d=eval_d, u_eval=u_eval,
...
def halving(levels, curve, eps: float) -> dict:
    ...
    if g0 <= eps:
        return dict(point=None, status="undefined", monotone=monotone,
                    residual=None, target=None)
    target = g0 / 2.0
    for i in range(len(levels) - 1):
        ...
        if (g_a - target) * (g_b - target) <= 0 and g_a != g_b:
            ...
            return dict(point=float(x), status="ok", monotone=monotone,
                        residual=None, target=float(target))
        if g_a == target:
            return dict(point=float(levels[i]), status="ok",
                        monotone=monotone, residual=None, target=float(target))
    return dict(point=None, status="beyond_range", monotone=monotone,
                residual=float(curve[-1] / g0), target=float(target))
...
# ------------------------------------------------------------ driver
def run_axis(cell: str, axis: str, seed: int, n_eval: int, n_tune: int,
             out_dir: str, cw: str = "mid", levels=None,
             n_boot: int = 2000) -> str:
    ...
    for lv in levels:
        ...
        advantage.append(dict(level=lv, mean=curve[lv], ci95=[lo, hi],
                              n_ep=int(len(counts))))
