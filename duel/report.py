...
def params_hash(resolved: dict) -> str:
    ...
    change the digest."""
    blob = json.dumps(jsonable(resolved), sort_keys=True, separators=(",", ":")).encode()
...
def envelope(
    kind: str, cell: str, seed: int, n_eval: int, n_tune: int, resolved: dict, payload: object
) -> dict:
    ...
    return dict(
        ...
        n_eval=n_eval,
        n_tune=n_tune,
        params_hash=params_hash(resolved),
        code=git_rev(),
