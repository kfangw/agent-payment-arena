...
def run_chain(env, rho, flow, seed, shape, n_tune, n_eval):
    ...
    best, _, _ = _tune_b4(lambda k, act: replay(env, tu, _B4Force(k, act), ex),
                          default_grids()["B3"], k_grid(env.N + 1), tu.pi0)
    from .b4 import B4
    b4 = B4(*best)
    a_pay = replay(env, ev, a, ex)
    episodes = np.arange(len(ev)) // CHAIN_BLOCK
    return _pack(a_pay, replay(env, ev, b1, ex), replay(env, ev, b4, ex),
                 episodes, float(np.mean(ev.v)), best)
...
def run_outage(env, flow, seed, shape, n_tune, n_eval):
    rng = np.random.default_rng(seed)
    tune_d, u_t = draw_outage_batch_crn(env, flow, n_tune, rng,
                                        payments_per_episode=50)
    eval_d, u_e = draw_outage_batch_crn(env, flow, n_eval, rng,
                                        payments_per_episode=50)
    ...
    best, _, _ = _tune_b4(
        ...
        default_grids()["B3"], k_grid(env.H), tu.pi0)
    b4 = OB4(best[0], best[1], best[2], env.H, env.N)
    a_pay = replay_outage(env, ev, a, ex)
    episodes = np.arange(len(ev)) // 50
    return _pack(a_pay, replay_outage(env, ev, b1, ex),
                 replay_outage(env, ev, b4, ex), episodes,
                 float(np.mean(ev.v)), best)
...
def _pack(a_pay, b1_pay, b4_pay, episodes, mexp, best):
    ...
    return dict(
        ...
        counts=counts, mexp=mexp, b4=dict(k=best[0], a=best[1], b=best[2]))
