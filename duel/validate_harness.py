...
Run:  python -m duel.validate_harness
"""
...
from .policies import (compile_A, default_grids, make_family_C, tune,
                       B1, PI_GRID)
...
def main():
    ...
    print(f"derived: q={der['q']:.5f} vstar={der['vstar']:.4f} "
    ...
    for rail, f in (('fast', [0.005] * 3), ('slow', [0.06, 0.03, 0.015])):
        ch = Channel(f=np.array(f), m=P['m'], h=P['h'], C=P['C'],
                     cw=P['cw'], tau=P['tau'], pmf_h=pmf)
        ...
        for name, pol in pols.items():
            ...
            print(f"  {name}: realized {r.mean():+.5f}  exact {e:+.5f}  "
                  f"z={z:+.2f}")
        ...
        e = exact['OPT'].mean()
        se = r.std(ddof=1) / np.sqrt(len(r))
        print(f"  A1: realized {r.mean():+.5f}  exact-opt {e:+.5f}  "
        ...
        print(f"  B1 tuned theta={best:.4f}: realized {rb.mean():+.5f}  "
