...
def _load(cell, results):
    ...
    return dict(
        ...
        k_holdout=hp["b4"]["k"],
        k0_gap=op["k0_identity_max_gap"], mean_exposure=op["mean_exposure"],
        split=op.get("tune_split"),
        base_hash=base["params_hash"], oracle_hash=orc["params_hash"],
...
def main(argv=None):
    ...
    hdr = ("cell", "A2", "B4", "B4orc", "k", "curse", "cbp", "A2-B4orc",
           "sim95", "bp", "verdict")
    print("%-14s %7s %7s %7s %3s %8s %6s %9s %18s %7s %s" % hdr)
    for r in rows:
        print("%-14s %7.3f %7.3f %7.3f %3d %8.4f %6.2f %9.4f [%7.4f,%7.4f] %7.2f %s"
              % (r["cell"], r["A2"], r["B4"], r["B4_oracle"], r["k"], r["curse"],
                 r["curse_bp"], r["diff"], r["ci"][0], r["ci"][1], r["bp"],
                 r["verdict"]))
    neg = [(r["cell"], r["curse"]) for r in rows if r["curse"] < -1e-12]
    print(f"\nB4_oracle >= B4 in all nine cells: {not neg}"
          + (f"  VIOLATIONS {neg}" if neg else ""))
    print(f"max winner's curse: {max(r['curse_bp'] for r in rows):.2f} bp")
    print(f"k=0 identity max gap over cells: "
          f"{max(r['k0_gap'] for r in rows):.3e}")
