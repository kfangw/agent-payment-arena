...
which proves the draws reproduce and, with them, A2.
    ...
    python -m duel.grid --env E-slow --flow F2 --seed 5 --out results
"""
...
from .outage import (draw_outage_batch, replay_outage, survival,
...
def _terminals_chain(ch, d, ex):
    return (replay(ch, d, B4Force(0, GRANT), ex),
            replay(ch, d, B4Force(0, REJECT), ex),
            replay(ch, d, B4Force(0, VERIFY), ex))


def _terminals_outage(env, d, ex):
    return (replay_outage(env, d, OB4Force(0, GRANT, env.H, env.N), ex),
...
def run_cell(env_name, flow_name, seed, n_tune, n_eval, results, ns):
    ...
    for n in ns:
        ...
        curve.append(dict(n=int(n), n_grid=len(grid), a=a, b=b,
                          tune_mean=tune_mean,
    ...
    payload = dict(
        curve=curve, ns=list(ns), k0_identity_max_gap=id_gap,
        n21_ab_matches_base=matched, mean_exposure=base['mean_exposure'],
        base_hash=base['base_hash'], base_code=base['base_code'],
...
def main(argv=None):
    ...
    payload, kind, env, rho = run_cell(args.env, args.flow, args.seed,
                                       args.n_tune, args.n_eval, args.results, ns)
    ...
    obj = envelope('grid', cell, args.seed, args.n_eval, args.n_tune,
                   resolved, payload)
    tag = f"{args.env}_{args.flow}_mid_s{args.seed}"
    path = str(Path(args.out) / f"duel_gridN_{tag}.json")
    write_once(path, obj)
    print(json.dumps(dict(
        cell=cell, id_gap=payload['k0_identity_max_gap'],
        n21_matches_base=payload['n21_ab_matches_base'],
        curve=[dict(n=r['n'], ab=[r['a'], r['b']],
                    a2_minus_b3=round(r['a2_minus_b3'], 6),
                    cost=round(r['coarseness_cost'], 6)) for r in payload['curve']],
    ), indent=1))
    print(f"wrote {path}")
