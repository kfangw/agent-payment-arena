...
Reads settlement envelopes in a results directory (never a pilot/ subtree),
...
Run:  python -m arena.experiments.settlement.aggregate --results results --out tables
"""
...
def load_cells(results_dir: str) -> dict:
    ...
    cells: dict = {}
    for path in sorted(Path(results_dir).glob("settlement_*.json")):
        env = json.loads(path.read_text())
        if env.get("kind") != "settlement":
            continue
        cell = env["cell"]
        pay = env["payload"]
