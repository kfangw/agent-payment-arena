...
Run:  python -m arena.experiments.settlement.acceptance
"""
...
def reference_suite(n=120_000, seed=20260823):
    ...
    run_case(
        "grant-all",
        ch1,
        Flow("w1", PointD([1.0], [1]), fixed_v(v)),
        ConstPol(GRANT),
        v * (1 - sig1 * (1 - h)),
    )
    ...
    run_case(
        "reject-all",
        ch2,
        Flow("w2", PointD([0.0], [1]), fixed_v(v)),
        ConstPol(REJECT),
        v * (sig2 * (1 + m) - 1),
    )
    ...
    run_case(
        "verify-all",
        ch3,
        Flow("w3", PointD([0.0], [1]), fixed_v(v3)),
        ConstPol(VERIFY),
        C + D0 - q * v3 * m,
    )
    ...
    d4 = draw_batch(ch4, Flow("w4", PointD([0.0, 1.0], [1, 1]), fixed_v(v)), n, rng)
    sig4 = sigma_list(f4)
    ex4 = np.maximum(sig4 * (1 + m) - 1.0, 0.0)
    blind = {a: replay(ch4, d4, ConstPol(a), ex4).mean() for a in (GRANT, REJECT, VERIFY)}
    ...
    results.append(
        ("amount-blind", float(gap4), float(bound4), float(se4), gap4 >= bound4 - 3 * se4)
    )
    ...
    run_case(
        "always-wait",
        ch5,
        Flow("w5", PointD([0.0], [1]), fixed_v(v)),
        WaitGrantPol(ch5.N + 1),
        bound5,
    )
    return results
...
def main():
    ...
    for name, gap, bound, se, passed in reference_suite():
        ok &= passed
        print(
            f"  {name:12s} gap {gap:+9.5f}  bound {bound:+9.5f}  "
