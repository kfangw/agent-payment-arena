...
region of the state space rather than stored per payment.
    ...
    python -m arena.experiments.settlement.residual --env E-outage --flow F2 --seed 8 --out results_r4
"""
...
def run_cell(env_name, flow_name, seed, n_tune, n_eval, results, results_grid):
    ...
    base = json.load(open(Path(results) / f"settlement_{env_name}_{flow_name}_mid_s{seed}.json"))
