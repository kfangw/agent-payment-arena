...
Run:  python -m arena.experiments.settlement.inject_b4_aggregate --results results_inject
"""
...
def rows_for(cell, axis, results):
    ...
    return dict(
        ...
        ident=ident,
        eps=pay["eps"],
        mean_exposure=pay["mean_exposure"],
        replicates=pay["replicates"],
        b4_level0=pay["b4_level0"],
        identity_mean=adv[ident]["mean"],
        table9=TABLE9[cell],
    )
...
def _fmt_row(a, eps):
    ...
    return (
        f"  lvl {a['level']:+6.2f}  A2-B4 {a['mean']:+.5f} "
...
def main(argv=None):
    ...
    for cell, axes in data.items():
        ...
        for ax, r in axes.items():
            ...
            print(
                f"[{ax}] identity A2-B4 {r['identity_mean']:.5f} vs Table9 "
        ...
        print(
            f"axis robustness (retention at far end, ascending = drops most "
