...
Run:  python -m arena.experiments.settlement.validate_aggregate
"""
...
def _write_cell(results: Path, cell: str, seed: int, advantage: float) -> None:
    """Fabricate one settlement envelope with block sums that put A2 above B1 by
    ...
    obj = envelope("settlement", cell, seed, n_ep * per, 50_000, dict(cell=cell), payload)
    write_once(str(results / f"settlement_{env}_{flow}_mid_s{seed}.json"), obj)
