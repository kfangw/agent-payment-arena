...
compiled tables, not a proof for the augmented state space.
    ...
    python -m arena.experiments.settlement.structcheck --env E-outage --flow F2 --seed 8 --out results_u1
"""
...
from .outage import OutageEnv, PI_GRID_OUTAGE, compile_outage, draw_outage_batch, survival
...
def _run_checks(state_iter, checks):
    ...
    for key, lab, sigma in state_iter:
        ...
        for c in names:
            if not res[c] and len(viol[c]) < 10:
                viol[c].append(
                    dict(state=key, sigma=round(float(sigma), 6), boundary=res["boundary"])
                )
    ...
    density = dict(
        n_single_action=n_single,
        n_c5_fired=n_c5_fired,
        max_boundary_dev=max_dev,
        label_presence=dict(
            grant=presence[0], reject=presence[1], verify=presence[2], wait=presence[3]
        ),
    )
...
def check_outage(env, flow_name, seed, n_tune):
    ...
    tune_d = draw_outage_batch(env, make_flows()[flow_name], n_tune, rng, payments_per_episode=50)
    ...
    n_ilr = (N + 2) * (H + 1) * 2
    density["max_boundary_dev_steps"] = density["max_boundary_dev"] * (len(PI_GRID_OUTAGE) - 1)
    return dict(
        kind="outage",
        flow_used=flow_name,
        n_states_checked=n,
...
def check_chain(ch, flow_name, seed, n_tune):
    ...
    n, out, density = _run_checks(it(), lambda lab, s: check_pi(lab, PI_GRID_CHAIN, s, ch.m, ch.h))
    density["max_boundary_dev_steps"] = density["max_boundary_dev"] * (len(PI_GRID_CHAIN) - 1)
    return dict(
        kind="chain",
        flow_used=flow_name,
        n_states_checked=n,
        n_stage=n_stage,
        n_v=pol.tables.shape[0],
        n_pi=len(PI_GRID_CHAIN),
        checks=out,
        density=density,
        q_hat=q,
    )
...
def main(argv=None):
    ...
    ap.add_argument("--env", required=True, choices=["E-outage", "E-fast", "E-slow"])
    ...
    payload = (
        check_outage(env, args.flow, args.seed, args.n_tune)
        if kind == "outage"
    ...
    obj = envelope(
        "structcheck", f"{args.env} x {args.flow}", args.seed, 0, args.n_tune, resolved, payload
    )
    ...
    print(
        json.dumps(
            dict(
                cell=payload["cell"],
                flow_used=payload["flow_used"],
                kind=payload["kind"],
                q_hat=round(payload["q_hat"], 5),
                states=payload["n_states_checked"],
                violations={c: payload["checks"][c]["violations"] for c in payload["checks"]},
                total=total_viol,
