...
Reads the base settlement_*.json for A2 block sums (family A is not recomputed),
...
Run:  python -m arena.experiments.settlement.grid_aggregate --results results
"""
...
def _load(cell, results, results_b4):
    ...
    seed = CELL_SEED[cell]
    base = json.load(open(Path(results) / f"settlement_{env}_{flow}_mid_s{seed}.json"))
    ...
    grid = json.load(open(Path(results) / f"settlement_gridN_{env}_{flow}_mid_s{seed}.json"))
...
def build(results="results", results_b4="results"):
    ...
    for r in rows:
        ...
        r["orig_ci"] = list(
            boot_ci(d0, r["counts"], n_boot=N_BOOT, seed=r["seed"], level=SIM_LEVEL)
        )
    ...
    for r, ph in zip(rows, holm(orig_perm)):
        r["orig_holm_p"] = ph
        r["orig_verdict"] = verdict(r["orig_ci"][0], r["orig_ci"][1], ph, r["eps"])
    return rows
...
def main(argv=None):
    ...
    hdr = ("cell", "A2", "B1", "B4", "k*", "A2-B4", "sim95", "bp", "holm_p", "verdict", "was(n21)")
    print("%-14s %7s %7s %7s %3s %8s %18s %7s %8s %-11s %s" % hdr)
    for r in rows:
        print(
            "%-14s %7.3f %7.3f %7.3f %3d %8.4f [%7.4f,%7.4f] %7.2f %8.4f %-11s %s"
            % (
                r["cell"],
                r["A2"],
                r["B1"],
    ...
    flips = [
        (r["cell"], r["orig_verdict"], r["verdict"])
        for r in rows
        if r["orig_verdict"] != r["verdict"]
    ]
