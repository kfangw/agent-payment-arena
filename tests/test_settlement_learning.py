...
def setting(hazards=(), **kwargs):
    return Setting(
        schedule=(((1.0, Request(0.05, hazards)),), ((1.0, Request(1.0)),)), **kwargs
    )


@pytest.mark.parametrize("hazards", [(), (0.1,), (0.2, 0.03)])
@pytest.mark.parametrize("rate", [0.0, 0.7, 1.0])
def test_last_request_matches_reference_actions(hazards, rate):
    s = replace(
        setting(response_rate=rate), schedule=(((1.0, Request(0.4, hazards)),),)
    )
    solver = Solver(s)
    ...
    reference = action_values(hazards, request.amount, [s.risk(0, 0)], p)
...
def test_eta_matches_independent_survival_sum():
...
def test_published_hand_calculation_and_uncertain_variant():
...
def test_observation_contract_is_idempotent_and_public():
    history = History()
    history.close(0, 0)
    history.close(0, 1)
...
def test_label_survives_when_conditional_release_is_unprofitable():
...
def tick_tree_value(s):
    ...
    def posterior(b, label):
        return (
            b * (s.high if label else 1 - s.high) / (risk(b) if label else 1 - risk(b))
        )

    def grant(r, stage, pi):
        survival = 1.0
    ...
    def query(n, r, stage, b, ticks):
        ...
        return (
            ...
            * (
                failure * unchanged
                + (1 - failure) * query(n, r, next_stage, b, ticks - 1)
            )
        )

    @cache
    def stage_value(n, r, stage, b):
        unchanged = future(n + 1, b)
        values = [
            unchanged,
...
def test_three_requests_against_tick_tree_and_baselines(rate):
    s = Setting(
        schedule=(
