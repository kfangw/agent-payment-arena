...
CELL_SEED = [("E-slow-deep", "F1", 10), ("E-slow-deep", "F2", 11),
             ("E-slow-deep", "F3", 12)]
...
def run_cell(env, flow, seed):
    ...
    if not (ROOT / "results" / f"duel_{tag}.json").exists():
        steps.append(["-m", "duel.run", "--env", env, "--flow", flow,
                      "--cw", "mid", "--n-eval", "5663400", "--n-tune",
                      "200000", "--seed", str(seed), "--out", "results"])
    if not (ROOT / "results" / f"b4_{tag}.json").exists():
        steps.append(["-m", "duel.b4", "--env", env, "--flow", flow,
                      "--seed", str(seed), "--k-grid", K_DENSE, "--out",
                      "results"])
    log = LOGS / f"s4_{tag}.log"
    with log.open("w") as fh:
        for step in steps:
            rc = subprocess.run([sys.executable, *step], cwd=ROOT, env=penv,
