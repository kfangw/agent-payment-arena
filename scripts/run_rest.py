...
S4_CELLS = [("E-slow-deep", "F1", 10), ("E-slow-deep", "F2", 11),
            ("E-slow-deep", "F3", 12)]
...
def _duel(env, flow, seed, out):
    return [PY, "-m", "duel.run", "--env", env, "--flow", flow, "--cw", "mid",
            "--n-eval", N_EVAL, "--n-tune", N_TUNE, "--seed", str(seed),
            "--out", out]
...
def jobs():
    ...
    for env, flow, seed in NINE:
        ...
        if _exists("results_s2", f"b4oracle_{tag}"):
            continue
        seq = [_duel(env, flow, seed, "results_s2"),
               [PY, "-m", "duel.b4", "--env", env, "--flow", flow, "--seed",
                str(seed), "--n-eval", N_EVAL, "--n-tune", N_TUNE,
                "--results", "results_s2", "--out", "results_s2"],
               [PY, "-m", "duel.b4", "--env", env, "--flow", flow, "--seed",
                str(seed), "--n-eval", N_EVAL, "--n-tune", N_TUNE, "--oracle",
                "--name", "b4oracle", "--results", "results_s2", "--out",
                "results_s2"]]
    ...
    for env, flow, seed in S3_CELLS:
        for axis in S3_AXES:
            ...
            if not _exists("results_inject", f"b4_{tag}"):
                out.append((f"s3_{tag}", [[PY, "-m", "duel.inject_b4", "--cell",
                            f"{env} x {flow}", "--axis", axis, "--n-eval",
                            N_EVAL, "--n-tune", N_TUNE, "--seed", str(seed),
                            "--out", "results_inject"]]))
    ...
    for env, flow, seed in S4_CELLS:
        ...
        if not _exists("results", f"b4_{tag}"):
            seq.append([PY, "-m", "duel.b4", "--env", env, "--flow", flow,
                        "--seed", str(seed), "--n-eval", N_EVAL, "--n-tune",
                        N_TUNE, "--k-grid", K_DENSE, "--out", "results"])
        if seq:
